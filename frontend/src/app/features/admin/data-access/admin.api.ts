...
export class AdminApi {
  ...
  resolveException(
    ...
    return this.http
      .post<ExceptionResponse>(`/api/v1/admin/exceptions/${exceptionId}/resolve`, request)
      .pipe(catchError((error) => throwError(() => mapHttpError(error))));
  }

  getPayrollSummary(): Observable<PayrollSummaryResponse> {
    return this.http
      .get<PayrollSummaryResponse>('/api/v1/admin/payroll/periods/current')
      .pipe(catchError((error) => throwError(() => mapHttpError(error))));
  }

  getPayrollPeriod(periodId: string): Observable<PayrollPeriodResponse> {
    return this.http
      .get<PayrollPeriodResponse>(`/api/v1/admin/payroll/periods/${periodId}`)
      .pipe(catchError((error) => throwError(() => mapHttpError(error))));
  }

  exportPayroll(periodId: string): Observable<Blob> {
    return this.http
      .get(`/api/v1/admin/payroll/periods/${periodId}/export`, { responseType: 'blob' })
      .pipe(catchError((error) => throwError(() => mapHttpError(error))));
  }

  getAuditLogs(): Observable<AuditLogResponse[]> {
    return this.http
      .get<AuditLogResponse[]>('/api/v1/admin/audit')
      .pipe(catchError((error) => throwError(() => mapHttpError(error))));
  }
}
