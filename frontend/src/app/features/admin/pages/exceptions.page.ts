...
import { AdminApi, ApiError, CrewResponse, ExceptionResponse } from '../data-access/admin.api';
...
export class AdminExceptionsPage {
  ...
  readonly resolveForm = this.formBuilder.nonNullable.group({
    ...
    action: ['APPROVE_AS_IS', [Validators.required]],
    checkInAt: [''],
    checkOutAt: [''],
    reason: [''],
    note: ['']
  ...
  loadCrews(): void {
    ...
    this.adminApi.getCrews().subscribe({
      next: (crews) => {
  ...
  loadExceptions(): void {
    ...
    this.loading.set(true);
    this.error.set(null);
    this.adminApi.getExceptions(date, crewId).subscribe({
      next: (items) => {
  ...
  onResolve(): void {
    ...
    this.loading.set(true);
    this.error.set(null);
    this.adminApi.resolveException(payload.exceptionId, request).subscribe({
      next: () => {
        this.loadExceptions();
      },
      error: (error: ApiError) => {
